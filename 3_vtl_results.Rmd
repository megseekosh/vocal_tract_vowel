...
```{r, measure deltaf dispersion via rms-1st time, include=FALSE}
...
vowels_rms <- delta_normed %>%
  ...
  mutate(rms_mean_sep_phones = round(mean(rms, na.rm=T),2)) %>%  # the mean rms value for each phone and age group
  mutate(rms_sd_sep_phones = round(sd(rms, na.rm=T),2))

# separate rms coefficient for each vowel and each *speaker*, for statistical modeling
vowels_rms_spkr <- delta_normed %>%
  ...
  mutate(rms = sqrt((f1_delta_norm - category_mean_F1)^2 +
               (f2_delta_norm - category_mean_F2)^2)) %>%
  mutate(rms_mean_sep_phones = round(mean(rms, na.rm=T),2)) %>%  # the mean rms value for each phone and spkr
  mutate(rms_sd_sep_phones = round(sd(rms, na.rm=T),2))
```
...
```{r, prepare group RMS tables, include=FALSE}
vowels_rms_tbl_alldata <- vowels_rms %>%  
  group_by(age_yrs, Phone) %>%
  select(age_yrs, Phone, rms) %>%
  distinct() %>%
  na.omit %>% # one 9;0 and one adult only have one [a] token over which rms was calculated so those emerge as NA
...
```{r, delta-disp-fig-RMS, fig.cap="Group-level RMS by age and phone", echo=FALSE}
delta_normed_plot_grp <- vowels_rms %>% #for group-level RMS
  filter(Phone!='o' & Phone!='e') %>%
  select(Age, Phone, rms_mean_sep_phones) %>%
  distinct() 

delta_normed_plot_spkr <- vowels_rms_spkr %>% # for individual RMS
  filter(Phone!='o' & Phone!='e') %>%
...
```{r, fit RMS models, include=FALSE}
...
```{r, prepare RMS model tbls, include=FALSE}
...
papaja::apa_table(rms_glmms %>% 
                    ...
                    select(-model), 
                  caption="Model predicting RMS for [a]") 
```
...
papaja::apa_table(rms_glmms %>% 
                    ...
                    select(-model), 
                  caption="Model predicting RMS for [i]") 
```
...
papaja::apa_table(rms_glmms %>% 
                    ...
                    select(-model), 
                  caption="Model predicting RMS for [u]") 
```
...
```{r, delta-disp-fig, fig.cap="Group-level RMS by age and phone", echo=FALSE}
