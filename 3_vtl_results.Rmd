...
```{r, count how many phones*age rmvd for ch4}
...
# calculate individual-level CoV 
delta_spkr_mean <- delta_normed %>%
  filter(Phone!='e' & Phone!='o') %>%
  group_by(Age, Phone, spkr) %>%    # calculate separately for each speaker, including adults
  mutate(F1_CoV = (sd(f1_delta_norm,na.rm=TRUE)) / (mean(f1_delta_norm,na.rm=TRUE))) %>% 
  mutate(F2_CoV = (sd(f2_delta_norm,na.rm=TRUE)) / (mean(f2_delta_norm,na.rm=TRUE)))
```
...
knitr::kable(delta_table_final, caption = 'Group-level average CoV by formant and vowel for children and adults', booktabs=T) %>% 
  kable_styling() %>%
...
delta_periph <- delta_normed_rmvd2 %>% 
  filter(Phone!='e' & Phone!='o') 
...
```{r, delta-disp-tbl-spkr-f1, echo=FALSE}
# we already have individual CoV calculated from above
delta_spkr_tbl <- delta_spkr_mean %>%  
  group_by(Age, Phone) %>%
  select(Age, Phone, F1_CoV, F2_CoV) %>%
  distinct() %>%
  na.omit %>% # one 9;0 and one adult only have one [a] token over which COV was calculated so those emerge as NA
  summarize(F1_mean=mean(F1_CoV),
            F1_sd=sd(F1_CoV),
            F1_min=min(F1_CoV),
            F1_max=max(F1_CoV),
            F2_mean=mean(F2_CoV),  
            ...
            F2_min=min(F2_CoV),
            F2_max=max(F2_CoV)) %>%
  mutate(F1_stats = paste(round(F1_mean,2), "(", round(F1_sd,2), ")", round(F1_min,2), "-", round(F1_max,2))) %>%
  mutate(F2_stats = paste(round(F2_mean,2), "(", round(F2_sd,2), ")", round(F2_min,2), "-", round(F2_max,2))) %>%
  select(-F1_min, -F2_min, -F1_max, -F2_max, -F1_sd, -F2_sd, -F1_mean, -F2_mean) 

# F1 table
delta_spkr_tbl_f1 <- delta_spkr_tbl %>%
  select(-F2_stats) %>%
  spread(key = "Phone", value = "F1_stats") 

# F2 table
delta_spkr_tbl_f2 <- delta_spkr_tbl %>%
  select(-F1_stats) %>%
  spread(key = "Phone", value = "F2_stats") 


colnames(delta_spkr_tbl_f1) <- c("Age","[a] mean (SD) range", "[i]", "[u]") # the 4;0 only have 1-2 tokens 
colnames(delta_spkr_tbl_f2) <- c("Age","[a] mean (SD) range", "[i]", "[u]") # the 4;0 only have 1-2 tokens 

delta_table_spkr_f1 <- data.table::setDT(delta_spkr_tbl_f1)
delta_table_spkr_f2 <- data.table::setDT(delta_spkr_tbl_f2)

#delta_table_spkr_f1 <- delta_table_spkr_f1[c(2, 3, 4, 5, 6, 7, 1, 8),]
#delta_table_spkr_f2 <- delta_table_spkr_f2[c(2, 3, 4, 5, 6, 7, 1, 8),]


knitr::kable(delta_table_spkr_f1, caption = 'Individual-level CoV by formant and vowel for children and adults:F1', booktabs=T) %>% 
  kable_styling() %>%
  kableExtra::kable_styling(latex_options = "hold_position")
```

```{r, delta-disp-tbl-spkr-f2, echo=FALSE}
knitr::kable(delta_table_spkr_f2, caption = 'Individual-level CoV by formant and vowel for children and adults:F2', booktabs=T) %>% 
  kable_styling() %>%
  kableExtra::kable_styling(latex_options = "hold_position")
```

```{r, delta-disp-fig, fig.cap="Group-level CoV by age and phone", echo=FALSE}
...
f1_a2 <- glmmTMB(F1_CoV ~ 
                gender + 
                (1|spkr), 
                data=a,  
                family="Gamma"(link="log"))
anova(f1_a, f1_a2) # doesn't improve
...
f2_a2 <- glmmTMB(F2_CoV ~ 
                gender + 
                (1|spkr), 
                data=a,  
                family="Gamma"(link="log"))
anova(f2_a, f2_a2) # doesn't improve under alpha .05
...
f1_i2 <- glmmTMB(F1_CoV ~ 
                gender + 
                (1|spkr), 
                data=i,  
                family="Gamma"(link="log"))
anova(f1_i, f1_i2) # doesn't improve 
...
f2_i2 <- glmmTMB(F2_CoV ~ 
                gender + 
                (1|spkr), 
                data=i,  
                family="Gamma"(link="log"))
summary(f2_i2) # not significant within model
...
f1_u2 <- glmmTMB(F1_CoV ~ 
                gender + 
                (1|spkr), 
                data=u,  
                family="Gamma"(link="log"))
anova(f1_u, f1_u2) # gender doesn't improve
...
f2_u2 <- glmmTMB(F2_CoV ~ 
                gender + 
                (1|spkr), 
                data=u,  
                family="Gamma"(link="log"))
anova(f2_u, f2_u2) # doesn't improve
...
Thus, although there are some trends apparent in the descriptive statistics of the individual-level and group-level CoVs, as well as in the vowel plots, overall the modeling suggests that category dispersion does not differ significantly between the adults and the child age groups, with the exception of the F2 of [i]. Furthermore, when there are statistically reliable differences between adults and children for within-category dispersion, these differences do not diminish with age.   
...
After scaling the vowels using the two scaling methods, the vowel variability between the children and adults for both sets of normalized vowels (Lobanov and $\Delta$F) was measured. Again, for the Lobanov normalization technique separate normalization factors are calculated for F1 and F2 for each speaker. The $\Delta$F technique, however, uses a single normalization coefficient for each speaker. We measured the variability of each vowel category for each age group (so three categories per age group) using a single coefficient that reflects both the *mean* value of each vowel category and the *variability* of each category along the F1 and F2 dimensions. To calculate this coefficient, the following steps were taken. 

1. First, we measured the difference between the mean value of each vowel category for the adults and a given child age group. For example, we measured the mean F1 of [a] over all of the 5;0 children and the mean F1 of [a] over all of the adults and then we took the difference between these values. This calculation was performed for the F1 and F2 measures for each vowel category. This step estimated the position of the vowel category in acoustic space. 

2. For the next step, we took the root mean square of the difference between the vowel categories for F1 and F2 (calculated in step one). Finally, these root mean square calculations for F1 and F2 were summed. This step estimated the standard deviation of the vowel category (disperion of the category in space).

These steps resulted in a single variability coefficient for each vowel category, for each child age group, that factored in both formants. Consequently, this measure is somewhat of an improvement upon the traditional CoV which has unique values for each formant. These measures were applied separately to the $\Delta$F-normalized vowels and the Lobanov-normalized vowels. 

```{r, measure deltaf dispersion via rms, include=FALSE}
# separate rms coefficient for each vowel and age group
...
delta_rms_kids <- delta_normed %>%
  filter(Age!='adult') %>%
  group_by(Age, a_articulator) %>% # calculate rms separately based on age and artic status
  mutate(mean_f1_a = mean(f1_delta_norm[Phone=='a'], na.rm=TRUE)) %>% 
  mutate(mean_f2_a = mean(f2_delta_norm[Phone=='a'], na.rm=TRUE)) %>% 
...
# get difference between means and calculate rms
...
```{r, measure lobanov dispersion via rms, include=FALSE}
# separate rms coefficient for each vowel and age group
# first calculate a mean for each formant and phone for the adults
...
# now calculate child rms and get difference between adult and child rms 
...
```{r, rms-dif-figure, fig.cap="Difference between adult and child vowel categories: RMS method", echo=FALSE}
...
# remove na outlier tokens
delta_all_adults <- delta_normed %>% 
  filter(Age=='adult') %>%
  select(f1_delta_norm, f2_delta_norm, f3_delta_norm, f4_delta_norm, spkr, Phone) %>%
  na.omit()  # remove nas (outlier measurements that we threw out)

delta_all_kids <- delta_normed %>% 
  filter(Age!='adult') %>%
  select(f1_delta_norm, f2_delta_norm, f3_delta_norm, spkr, Phone) %>%
  na.omit()  # remove nas (outlier measurements that we threw out)
...
delta_normed_all <- merge(delta_all_all, vls_normed_rmvd2, by = c("spkr", "Phone", "f1_delta_norm", "f2_delta_norm", "f3_delta_norm", "f4_delta_norm"))

# first measure group and individual CoV over all data
# we only calculate on peripheral vowels
periph_delta_all <- delta_normed_all %>% filter(Phone !='e' & Phone !='o')
# calculate group-level CoV
periph_delta_all <- periph_delta_all %>%
  group_by(Age, Phone) %>%    # calculate separately for each age group, including adults, and vowel
  mutate(F1_CoV = (sd(f1_delta_norm)) / (mean(f1_delta_norm))) %>% 
  mutate(F2_CoV = (sd(f2_delta_norm)) / (mean(f2_delta_norm))) 

# calculate individual-level CoV 
delta_spkr_mean_alldata <- periph_delta_all %>%
  group_by(Age, Phone, spkr) %>%    # calculate separately for each speaker, including adults
  mutate(F1_CoV = (sd(f1_delta_norm)) / (mean(f1_delta_norm))) %>% 
  mutate(F2_CoV = (sd(f2_delta_norm)) / (mean(f2_delta_norm)))

# for F1
delta_COV_table_f1_alldata <- periph_delta_all %>%
  select(Age, Phone, F1_CoV) %>%
  distinct() %>%
  spread(key = "Phone", value = "F1_CoV") %>%
  mutate(a=round(a,2),
...
# for F2
delta_COV_table_f2_alldata <- periph_delta_all %>%
  select(Age, Phone, F2_CoV) %>%
  distinct() %>%
  spread(key = "Phone", value = "F2_CoV") %>%
  mutate(a=round(a,2),
...
delta_table_final_alldata <- merge(delta_COV_table_f1_alldata, delta_COV_table_f2_alldata, by=c("Age"))

colnames(delta_table_final_alldata) <- c("Age","[a]", "[i]", "[u]", "[a] ", "[i] ", "[u] ")
delta_table_final_alldata <- data.table::setDT(delta_table_final_alldata)

delta_table_final_alldata <- delta_table_final_alldata[c(2, 3, 4, 5, 6, 7, 1, 8),]


knitr::kable(delta_table_final_alldata, caption = 'Group-level average CoV by formant and vowel for children and adults: all data', booktabs=T) %>% 
  kable_styling() %>%
  add_header_above(c(" " = 1, "F1" = 3, "F2" = 3)) %>%
  kableExtra::kable_styling(latex_options = "hold_position")
```

```{r, delta-disp-tbl-spkr-alldata-f1, echo=FALSE}
# we already have individual CoV calculated from above
delta_spkr_tbl_alldata <- delta_spkr_mean_alldata %>%  
  group_by(Age, Phone) %>%
  select(Age, Phone, F1_CoV, F2_CoV) %>%
  distinct() %>%
  na.omit %>% # one 9;0 and one adult only have one [a] token over which COV was calculated so those emerge as NA
...
# F1 table
delta_spkr_tbl_f1_alldata <- delta_spkr_tbl_alldata %>%
  select(-F2_stats) %>%
  spread(key = "Phone", value = "F1_stats") 

# F2 table
delta_spkr_tbl_f2_alldata <- delta_spkr_tbl_alldata %>%
  select(-F1_stats) %>%
  spread(key = "Phone", value = "F2_stats") 

colnames(delta_spkr_tbl_f2_alldata) <- c("Age","[a] mean (SD) range", "[i]", "[u]") # the 4;0 only have 1-2 tokens 
colnames(delta_spkr_tbl_f1_alldata) <- c("Age","[a] mean (SD) range", "[i]", "[u]") # the 4;0 only have 1-2 tokens 

delta_table_spkr_f2 <- data.table::setDT(delta_spkr_tbl_f2_alldata)
delta_table_spkr_f1 <- data.table::setDT(delta_spkr_tbl_f1_alldata)

#delta_table_spkr_f2 <- delta_table_spkr_f2[c(2, 3, 4, 5, 6, 7, 8, 1),]
#delta_table_spkr_f1 <- delta_table_spkr_f1[c(2, 3, 4, 5, 6, 7, 8, 1),]
...
```{r, delta-disp-tbl-spkr-alldata-f2, echo=FALSE}
knitr::kable(delta_table_spkr_f2, caption = 'Individual-level CoV by formant and vowel for children and adults: F2', booktabs=T) %>% 
  kable_styling() %>%
  kableExtra::kable_styling(latex_options = "hold_position")
```
